...
bool RarVM::ExecuteCode(VM_PreparedCommand *PreparedCode,uint CodeSize)
{
  ...
  while (1)
  {
    ...
    switch(Cmd->OpCode)
    {
      ...
      case VM_CALL:
        R[7]-=4;
        SET_VALUE(false,(uint *)&Mem[R[7]&VM_MEMMASK],static_cast<uint>(Cmd-PreparedCode+1));
        SET_IP(GET_VALUE(false,Op1));
        continue;
      case VM_NOT:
...
void RarVM::Optimize(VM_PreparedProgram *Prg)
{
  ...
  for (uint I=0;I<CodeSize;I++)
  {
    ...
    switch(Cmd->OpCode)
    {
      ...
      default:
          // mangofix: tons of missing case statements
          break;
    }
...
VM_StandardFilters RarVM::IsStandardFilter(byte *Code,uint CodeSize)
{
  ...
  for (uint I=0;I<ASIZE(StdList);I++)
    if (StdList[I].CRC==CodeCRC && uint(StdList[I].Length)==CodeSize)
...
void RarVM::ExecuteStandardFilter(VM_StandardFilters FilterType)
{
  ...
  {
    case VMSF_NONE:
      // mangofix: missing case statement
      break;
    case VMSF_E8:
    case VMSF_E8E9:
      {
