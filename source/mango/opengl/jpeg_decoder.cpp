...
void OpenGLJPEGDecoder::decode(s16* data, int xmcu, int ymcu)
{
    size_t elements = ymcu * xmcu * blocks_in_mcu * 64;
    size_t bytes = elements * sizeof(s16);

    GLuint sbo;
    glGenBuffers(1, &sbo);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, sbo);
    glBufferData(GL_SHADER_STORAGE_BUFFER, bytes, reinterpret_cast<GLvoid*>(data), GL_DYNAMIC_COPY);
    ...
    for (int i = 0; i < 64; ++i)
    {
        quantize[i + 64 * 0] = qt[0][i];
    }

    for (int i = 0; i < 64; ++i)
    {
        quantize[i + 64 * 1] = qt[1][i];
    }

    for (int i = 0; i < 64; ++i)
    {
        quantize[i + 64 * 2] = qt[2][i];
    }
...
GLuint OpenGLJPEGDecoder::decode(ConstMemory memory)
{
    ...
    image::ImageDecodeStatus status = parser.decode(this, options);
