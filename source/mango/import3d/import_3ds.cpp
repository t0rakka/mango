...
struct Mesh3DS
{
    ...
    std::vector<float2> mappings;
    std::vector<Face3DS> faces;

    float3 computeNormal(const Face3DS& face) const
    {
        float3 p0 = positions[face.index[0]];
        float3 p1 = positions[face.index[1]];
        float3 p2 = positions[face.index[2]];
...
public:
    ...
    float3 read_vec3(LittleEndianConstPointer& p) const
    {
        float x = p.read32f();
        float y = p.read32f();
        float z = p.read32f();
    ...
    float2 read_vec2(LittleEndianConstPointer& p) const
    {
        float x = p.read32f();
        float y = p.read32f();
        return float2(x, y);
    ...
    void chunk_master_scale(LittleEndianConstPointer& p)
    {
        ...
        print("master scale: %f", master_scale);
    }
	...
	void chunk_mesh_version(LittleEndianConstPointer& p)
    {
        ...
        print("mesh version: %d", mesh_version);
    }
    ...
    void chunk_material_name(LittleEndianConstPointer& p)
    {
        ...
        print("name: \"%s\"", material.name.c_str());
    }
    ...
    void chunk_material_ambient_color(LittleEndianConstPointer& p)
    {
        ...
        print("ambient color: %d, %d, %d", color[0], color[1], color[2]);
    }
    ...
    void chunk_material_diffuse_color(LittleEndianConstPointer& p)
    {
        ...
        print("diffuse color: %d, %d, %d", color[0], color[1], color[2]);
    }
    ...
    void chunk_material_specular_color(LittleEndianConstPointer& p)
    {
        ...
        print("specular color: %d, %d, %d", color[0], color[1], color[2]);
    }
    ...
    void chunk_material_wireframe_size(LittleEndianConstPointer& p)
    {
        ...
        print("wireframe size: %f", percent);
    }
    ...
    void chunk_texture_map_name(LittleEndianConstPointer& p)
    {
        ...
        print("filename: \"%s\"", texture->filename.c_str());
    }
    ...
    void chunk_texture_map_tiling(LittleEndianConstPointer& p)
    {
        ...
        print("tiling: 0x%x", tiling_flags);
    }
    ...
    void chunk_mesh_vertex_list(LittleEndianConstPointer& p)
    {
        ...
        {
            float3 v = read_vec3(p);
            mesh.positions[i] = float3(-v.x, v.z, -v.y);
        }
    }
    ...
    void chunk_mesh_material_list(LittleEndianConstPointer& p)
    {
        ...
        int count = p.read16();
        print("material.list: %d, name: \"%s\"", count, name.c_str());
    ...
    void chunk_mesh_mapping_list(LittleEndianConstPointer& p)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            float2 v = read_vec2(p);
            mesh.mappings[i] =  float2(v.x, -v.y);
        }
    }
    ...
    {
        float3 position = read_vec3(p);
        float3 target = read_vec3(p);
...
Import3DS::Import3DS(File& file)
{
    Reader3DS reader(file);

    reader.print("materials: %d", int(reader.materials.size()));
    reader.print("meshes: %d", int(reader.meshes.size()));
    ...
    for (auto& mesh3ds : reader.meshes)
    {
        ...
        std::multimap<float3, u32, compare> sharing;
