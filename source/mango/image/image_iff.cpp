...
namespace
{
    ...
    bool read_signature(u8*& data)
    {
		...
		if (v0 != make_u32be('F','O','R','M'))
            MANGO_EXCEPTION(ID"Incorrect signature.");

		if (v1 != make_u32be('I','L','B','M') && v1 != make_u32be('P','B','M',' '))
            MANGO_EXCEPTION(ID"Incorrect signature.");

        return v1 == make_u32be('P','B','M',' ');
    }
