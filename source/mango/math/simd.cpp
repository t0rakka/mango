...
namespace simd {
    ...
    float64x4 atan2(float64x4 y, float64x4 x)
    {
        ...
        r = select(float64x4_or(is_inf(x), compare_eq(x, float64x4_zero())),
                             sub(pi_2, is_inf2(x, mulsign(pi_2, x))), r);
        r = select(is_inf(y), 
                             sub(pi_2, is_inf2(x, mulsign(pi_4, x))), r);
        r = select(compare_eq(y, float64x4_zero()),
                             float64x4_and(compare_eq(signed_one(x), float64x4_set1(-1.0)), pi), r);
