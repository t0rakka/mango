...
namespace mango::vulkan
{
    ...
    namespace
    {
        ...
        // Multiple encode() calls per beginFrame() (e.g. UI window layers) each need a
        // distinct upload slot — otherwise later encodes clobber staging buffers that
        // earlier dispatches in the same command buffer will read at submit time.
        static constexpr u32 kMaxEncodesPerFrame = 16;
        static constexpr u32 kMaxBatchSlots = kMaxFramesInFlight * kMaxEncodesPerFrame;
                                                   ...
                                                   float32x4 color, float stemDarkening)
        {
            ...
            // Keep float screen origins for subpixel pen alignment, but size the
            // raster window to the enclosing integer pixel rect so partial rows at
            // the top/left edges are not rejected by gid < 0 in the shader.
            const float bbox_left = std::floor(float_left);
            const float bbox_top = std::floor(float_top);
            const float bbox_right = std::ceil(float_right);
            const float bbox_bottom = std::ceil(float_bottom);

            GlyphDrawParams params;
            params.screen_x = float_left;
            params.screen_y = float_top;
            params.em_window_x0 = cached.cpu.bounds_min.x;
            params.em_window_y0 = (baseline_y - float_top - 1.0f) * em_per_pixel;
            params.em_per_pixel = em_per_pixel;
            params.pixel_width = std::max(1u, u32(bbox_right - bbox_left));
            params.pixel_height = std::max(1u, u32(bbox_bottom - bbox_top));
            params.color = color;
    ...
    struct FontRenderer::Impl
    {
        ...
        VkImageView boundTargetView = VK_NULL_HANDLE;
        VkDescriptorSet batchDescriptorSets[kMaxBatchSlots] = {};
        ...
        BufferAllocation instanceBuffers[kMaxBatchSlots];
        BufferAllocation tileBuffers[kMaxBatchSlots];
        BufferAllocation tileGlyphBuffers[kMaxBatchSlots];
        VkDeviceSize instanceBufferCapacity[kMaxBatchSlots] = {};
        VkDeviceSize tileBufferCapacity[kMaxBatchSlots] = {};
        VkDeviceSize tileGlyphBufferCapacity[kMaxBatchSlots] = {};
        static constexpr u32 kFramesInFlight = 2;
        u32 framesInFlight = kFramesInFlight;
        u32 batchSlotCount = kFramesInFlight * kMaxEncodesPerFrame;
        u32 encodesInFrame = 0;
        ...
        Impl(VkDevice dev, VkQueue q, u32 family, Allocator& alloc)
            ...
            , batchSlotCount(kFramesInFlight * kMaxEncodesPerFrame)
            , encodesInFrame(0)
        {
        ...
        void cacheGlyph(u32 faceIndex, u32 id, float pixelHeight, font::Hinting hinting, bool by_glyph_index)
        {
            const u64 key = glyphCacheKey(faceIndex, id, hinting, pixelHeight, by_glyph_index);
            ...
            CachedGlyph cached;
            cached.cpu = by_glyph_index
                ? slot.face.loadGpuGlyphByIndex(id)
                : slot.face.loadGpuGlyph(id);
        ...
        const CachedGlyph& getCachedGlyph(u32 faceIndex, u32 id, float pixelHeight, font::Hinting hinting, bool by_glyph_index)
        {
            const u64 key = glyphCacheKey(faceIndex, id, hinting, pixelHeight, by_glyph_index);
            auto it = glyphCache.find(key);
            if (it == glyphCache.end())
            {
                cacheGlyph(faceIndex, id, pixelHeight, hinting, by_glyph_index);
                                ...
                                float stemDarkening, FontHinting hinting)
        {
            ...
            withFaceAtHeight(faceIndex, pixelHeight, fontHinting, [&](font::Face& face, float height)
            {
                ...
                const font::ShapedRun shaped = font::shapeText(face, codepoints);
                const bool by_glyph_index = font::textShapingEnabled();

                float cursor_x = pen_x;
                for (const font::ShapedGlyph& glyph : shaped.glyphs)
                {
                    const u32 cache_id = by_glyph_index ? glyph.glyph_index : glyph.cluster < codepoints.size()
                        ? u32(codepoints[glyph.cluster]) : glyph.glyph_index;
                    const CachedGlyph& cached = getCachedGlyph(faceIndex, cache_id, height, fontHinting, by_glyph_index);

                    if (!cached.cpu.curve_bytes.empty())
                    {
                        queueGlyph(cached.atlas,
                            makeGlyphDrawParams(cursor_x + glyph.x_offset, baseline_y + glyph.y_offset,
                                cached, pixel_scale, em_per_pixel, color, stemDarkening));
                    }
        ...
        void queueGlyph(const GlyphAtlasSlot& atlas, const GlyphDrawParams& params)
        {
            ...
            {
                std::floor(params.screen_x),
                std::floor(params.screen_y),
                std::floor(params.screen_x) + float(pixel_width),
                std::floor(params.screen_y) + float(pixel_height),
            });
        }
                                      ...
                                      float pixelHeight, float stemDarkening, FontHinting hinting)
        {
            ...
            withFaceAtHeight(faceIndex, pixelHeight, fontHinting, [&](font::Face& face, float height)
            {
                ...
                {
                    const bool by_glyph_index = font::textShapingEnabled();
                    const u32 cache_id = by_glyph_index ? pg.glyph_index : pg.codepoint;
                    const CachedGlyph& cached = getCachedGlyph(faceIndex, cache_id, height, fontHinting, by_glyph_index);
        ...
        void resize(VkExtent2D newExtent)
        {
            ...
            vkDeviceWaitIdle(device);
            extent = newExtent;
            boundTargetView = VK_NULL_HANDLE;
            reserveBatchBuffers(newExtent);
        }

        void bindTargetView(VkImageView imageView)
        {
            if (!imageView || !targetDescriptorSet)
            {
                return;
            }

            if (imageView == boundTargetView)
            {
                return;
            }

            vkDeviceWaitIdle(device);
            boundTargetView = imageView;
            writeTargetDescriptor(targetDescriptorSet, imageView);
        }

        void writeTargetDescriptor(VkDescriptorSet descriptorSet, VkImageView imageView)
        {
            if (!descriptorSet || !imageView)
            {
                return;
            }
        ...
        static void batchCapacitiesForExtent(VkExtent2D extent,
            VkDeviceSize& instance_bytes, VkDeviceSize& tile_bytes, VkDeviceSize& index_bytes)
        {
            const u32 tiles_x = (extent.width + kTileSize - 1) / kTileSize;
            const u32 tiles_y = (extent.height + kTileSize - 1) / kTileSize;
            const u32 max_tiles = tiles_x * tiles_y;
            const u32 max_glyphs = 8192;
            const u32 max_indices = std::max(max_glyphs * 32u, max_tiles * 16u);

            instance_bytes = sizeof(GpuGlyphInstance) * max_glyphs;
            tile_bytes = sizeof(GpuTileInfo) * std::max(max_tiles, 1u);
            index_bytes = sizeof(u32) * max_indices;
        }

        void reserveBatchBuffers(VkExtent2D targetExtent)
        {
            VkDeviceSize instance_bytes = 0;
            VkDeviceSize tile_bytes = 0;
            VkDeviceSize index_bytes = 0;
            batchCapacitiesForExtent(targetExtent, instance_bytes, tile_bytes, index_bytes);

            for (u32 slot = 0; slot < batchSlotCount; ++slot)
            {
                ensureBatchBuffer(slot, instanceBuffers[slot], instanceBufferCapacity[slot], instance_bytes);
                ensureBatchBuffer(slot, tileBuffers[slot], tileBufferCapacity[slot], tile_bytes);
                ensureBatchBuffer(slot, tileGlyphBuffers[slot], tileGlyphBufferCapacity[slot], index_bytes);
            }
        }
        ...
        TextBounds computePendingBounds(VkExtent2D clip_extent) const
        {
            ...
            // Clip to the target in float space first. Casting a negative ceil() to u32
            // (text fully above/left of the image) used to wrap to ~4e9 and blow up tiling.
            const float clipped_x0 = std::max(0.0f, std::floor(x0));
            const float clipped_y0 = std::max(0.0f, std::floor(y0));
            const float clipped_x1 = std::min(float(clip_extent.width), std::ceil(x1));
            const float clipped_y1 = std::min(float(clip_extent.height), std::ceil(y1));
            if (clipped_x1 <= clipped_x0 || clipped_y1 <= clipped_y0)
            {
                return bounds;
            }

            bounds.x0 = u32(clipped_x0);
            bounds.y0 = u32(clipped_y0);
            bounds.x1 = u32(clipped_x1);
            bounds.y1 = u32(clipped_y1);
            return bounds;
        }

        void flushTextBatch(VkCommandBuffer cmd, const TextBounds& bounds, u32 frameIndex)
        {
            if (pendingInstances.empty() || bounds.empty() || !targetDescriptorSet)
            {
                return;
            }
            ...
            for (u32 i = 0; i < glyph_count; ++i)
            {
                ...
                // Glyphs fully outside the clipped tile grid contribute nothing.
                if (rect.x1 <= origin_x || rect.y1 <= origin_y ||
                    rect.x0 >= float(bounds.x1) || rect.y0 >= float(bounds.y1))
                {
                    range.tx0 = 1;
                    range.ty0 = 1;
                    range.tx1 = 0;
            ...
            if (encodesInFrame >= kMaxEncodesPerFrame)
            {
                printLine(Print::Error,
                    "FontRenderer: too many encode() calls in one frame (max {}); later text batches dropped",
                    kMaxEncodesPerFrame);
                return;
            }

            // Per-frame encode index so layered UI can encode multiple times without
            // clobbering host-visible staging that earlier dispatches still reference.
            const u32 slot = (frameIndex % framesInFlight) * kMaxEncodesPerFrame + encodesInFrame;
            ++encodesInFrame;
            ...
            if (instance_bytes > instanceBufferCapacity[slot] ||
                tile_bytes > tileBufferCapacity[slot] ||
                index_bytes > tileGlyphBufferCapacity[slot])
            {
                printLine(Print::Error,
                    "FontRenderer: batch buffer overflow ({} instance bytes, {} tile bytes, {} index bytes); call resize() for the current extent",
                    instance_bytes, tile_bytes, index_bytes);
                return;
            }
            ...
            VkDescriptorSet sets[] = { targetDescriptorSet, batchDescriptorSets[slot] };
        ...
        void createDescriptorSetLayouts()
        {
            ...
            VkDescriptorPoolSize targetPoolSize = { VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1 };
            ...
            VkDescriptorSetAllocateInfo targetAllocInfo =
            {
            ...
            vkAllocateDescriptorSets(device, &targetAllocInfo, &targetDescriptorSet);

            VkDescriptorPoolSize batchPoolSizes[] =
            {
                { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 5 * batchSlotCount },
            };
            ...
            VkDescriptorSetLayout batchLayouts[kMaxBatchSlots] = {};
            for (u32 i = 0; i < batchSlotCount; ++i)
            {
            ...
            VkDescriptorSetAllocateInfo batchAllocInfo =
            {
                ...
                .descriptorSetCount = batchSlotCount,
        ...
        void updateAtlasDescriptors()
        {
            ...
            VkWriteDescriptorSet writes[kMaxBatchSlots * 2] = {};
            for (u32 i = 0; i < batchSlotCount; ++i)
            {
                writes[i * 2 + 0] =
                {
            ...
            vkUpdateDescriptorSets(device, batchSlotCount * 2, writes, 0, nullptr);
        }
    ...
    void FontRenderer::setSize(Font font, float pixelHeight)
    {
        ...
        const float h = std::max(1.0f, pixelHeight);
        if (std::abs(slot->pixelHeight - h) < 1e-4f)
        {
            return;
        }

        slot->pixelHeight = h;
        slot->face.setPixelHeight(h);
    }
    ...
    void FontRenderer::warmup(Font font, std::string_view utf8)
    {
        ...
        m_impl->withFaceAtHeight(font.index, slot->pixelHeight, font::Hinting::None, [&](font::Face& face, float height)
        {
            const font::ShapedRun shaped = font::shapeText(face, codepoints);
            const bool by_glyph_index = font::textShapingEnabled();
            for (const font::ShapedGlyph& glyph : shaped.glyphs)
            {
                const u32 cache_id = by_glyph_index ? glyph.glyph_index
                    : (glyph.cluster < codepoints.size() ? u32(codepoints[glyph.cluster]) : glyph.glyph_index);
                m_impl->cacheGlyph(font.index, cache_id, height, font::Hinting::None, by_glyph_index);
            }
        });
    }
    ...
    void FontRenderer::bindTarget(VkImageView imageView)
    {
        m_impl->bindTargetView(imageView);
    }
    ...
    void FontRenderer::encode(VkCommandBuffer cmd, const EncodeTarget& target)
    {
        ...
        if (target.imageView != m_impl->boundTargetView)
        {
            printLine(Print::Error, "FontRenderer::encode: target image view is not bound; call bindTarget() after creating or recreating the render target.");
            return;
        }
        ...
        m_impl->flushTextBatch(cmd, bounds, target.frameIndex);
        m_impl->pendingInstances.clear();
        m_impl->pendingRects.clear();
    }
