...
//                        pPage->JPCI[i].v_samp_factor = ucSamp & 0xf;
            ...
            case 0xffdb: /* M_DQT */
                ...
                while (usLen > 0)
                {
                    ...
                    iTableOffset = (ucTable & 0xf) * X_DCTSIZE;
                    if (ucTable & 0xf0) // if word precision
                    {
                        for (i=0; i<X_DCTSIZE; i++)
                        {
...
static void JPEGFixQuantD(JPEGIMAGE *pJPEG)
{
    ...
    signed short sTemp[X_DCTSIZE];
    int i;
    uint16_t *p;
    
    for (iTable=0; iTable<pJPEG->ucNumComponents; iTable++)
    {
        iTableOffset = iTable * X_DCTSIZE;
        p = (uint16_t *)&pJPEG->sQuantTable[iTableOffset];
        for (i=0; i<X_DCTSIZE; i++)
            sTemp[i] = p[cZigZag[i]];
        memcpy(&pJPEG->sQuantTable[iTableOffset], sTemp, X_DCTSIZE*sizeof(short)); // copy back to original spot
...
static void JPEGIDCT(JPEGIMAGE *pJPEG, int iMCUOffset, int iQuantTable, int iACFlags)
{
    ...
    pQuant = &pJPEG->sQuantTable[iQuantTable * X_DCTSIZE];
...
static void JPEGPutMCU11(JPEGIMAGE *pJPEG, int x, int iPitch)
{
    ...
    pY  = (unsigned char *)&pJPEG->sMCUs[0*X_DCTSIZE];
    pCb = (unsigned char *)&pJPEG->sMCUs[1*X_DCTSIZE];
    pCr = (unsigned char *)&pJPEG->sMCUs[2*X_DCTSIZE];
...
static void JPEGPutMCU22(JPEGIMAGE *pJPEG, int x, int iPitch)
{
    ...
    pY  = (unsigned char *)&pJPEG->sMCUs[0*X_DCTSIZE];
    pCb = (unsigned char *)&pJPEG->sMCUs[4*X_DCTSIZE];
    pCr = (unsigned char *)&pJPEG->sMCUs[5*X_DCTSIZE];
    ...
    if (pJPEG->iOptions & JPEG_SCALE_HALF) // special handling of 1/2 size (pixel averaging)
    {
        for (iRow=0; iRow<4; iRow++) // 16x16 becomes 8x8 of 2x2 pixels
        {
            for (iCol=0; iCol<4; iCol++)
            {
                ...
                Y1 = (pY[iCol*2+(X_DCTSIZE*2)] + pY[iCol*2+1+(X_DCTSIZE*2)] + pY[iCol*2+8+(X_DCTSIZE*2)] + pY[iCol*2+9+(X_DCTSIZE*2)]) << 10;
                Cb = pCb[iCol+4];
                Cr = pCr[iCol+4];
                if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
                ...
                Y1 = (pY[iCol*2+(X_DCTSIZE*4)] + pY[iCol*2+1+(X_DCTSIZE*4)] + pY[iCol*2+8+(X_DCTSIZE*4)] + pY[iCol*2+9+(X_DCTSIZE*4)]) << 10;
                Cb = pCb[iCol+32];
                Cr = pCr[iCol+32];
                if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
                ...
                Y1 = (pY[iCol*2+(X_DCTSIZE*6)] + pY[iCol*2+1+(X_DCTSIZE*6)] + pY[iCol*2+8+(X_DCTSIZE*6)] + pY[iCol*2+9+(X_DCTSIZE*6)]) << 10;
                Cb = pCb[iCol+32+4];
                Cr = pCr[iCol+32+4];
                if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
    ...
    if (pJPEG->iOptions & JPEG_SCALE_EIGHTH)
    {
        ...
        // top right block
        Y1 =  pY[X_DCTSIZE*2] << 12; // scale to level of conversion table
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
            JPEGPixelLE(pOutput + 1, Y1, Cb, Cr);
        else
            JPEGPixelBE(pOutput + 1, Y1, Cb, Cr);
        // bottom left block
        Y1 =  pY[X_DCTSIZE*4] << 12;  // scale to level of conversion table
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
            JPEGPixelLE(pOutput+iPitch, Y1, Cb, Cr);
        else
            JPEGPixelBE(pOutput+iPitch, Y1, Cb, Cr);
        // bottom right block
        Y1 =  pY[X_DCTSIZE*6] << 12; // scale to level of conversion table
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
        ...
        for (iRow=0; iRow<2; iRow++)
        {
            if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
            {
                for (iCol=0; iCol<2; iCol++)
                {
                    ...
                    // top right block
                    Y1 =  pY[iCol+(X_DCTSIZE*2)] << 12; // scale to level of conversion table
                    Cb = pCb[1];
                    Cr = pCr[1];
                    JPEGPixelLE(pOutput + 2+iCol, Y1, Cb, Cr);
                    // bottom left block
                    Y1 =  pY[iCol+X_DCTSIZE*4] << 12;  // scale to level of conversion table
                    Cb = pCb[2];
                    Cr = pCr[2];
                    JPEGPixelLE(pOutput+iPitch*2 + iCol, Y1, Cb, Cr);
                    // bottom right block
                    Y1 =  pY[iCol+X_DCTSIZE*6] << 12; // scale to level of conversion table
                    Cb  = pCb[3];
                    Cr  = pCr[3];
    ...
    for (iRow=0; iRow<iYCount; iRow++) // up to 4 rows to do
    {
        for (iCol=0; iCol<iXCount1; iCol++) // up to 4 cols to do
        {
            ...
            if (iCol < iXCount2)
            {
                Y1 = pY[iCol*2+X_DCTSIZE*2];
                Y2 = pY[iCol*2+1+X_DCTSIZE*2];
                Y3 = pY[iCol*2+8+X_DCTSIZE*2];
                Y4 = pY[iCol*2+9+X_DCTSIZE*2];
                Y1 <<= 12;  // scale to level of conversion table
                Y2 <<= 12;
                Y3 <<= 12;
                Y4 <<= 12;
            ...
            Y1 = pY[iCol*2+X_DCTSIZE*4];
            Y2 = pY[iCol*2+1+X_DCTSIZE*4];
            Y3 = pY[iCol*2+8+X_DCTSIZE*4];
            Y4 = pY[iCol*2+9+X_DCTSIZE*4];
            Y1 <<= 12;  // scale to level of conversion table
            Y2 <<= 12;
            Y3 <<= 12;
            Y4 <<= 12;
            ...
            if (iCol < iXCount2)
            {
                Y1 = pY[iCol*2+X_DCTSIZE*6];
                Y2 = pY[iCol*2+1+X_DCTSIZE*6];
                Y3 = pY[iCol*2+8+X_DCTSIZE*6];
                Y4 = pY[iCol*2+9+X_DCTSIZE*6];
                Y1 <<= 12;  // scale to level of conversion table
                Y2 <<= 12;
                Y3 <<= 12;
                Y4 <<= 12;
...
static void JPEGPutMCU12(JPEGIMAGE *pJPEG, int x, int iPitch)
{
    ...
    pY  = (uint8_t *)&pJPEG->sMCUs[0*X_DCTSIZE];
    pCb = (uint8_t *)&pJPEG->sMCUs[2*X_DCTSIZE];
    pCr = (uint8_t *)&pJPEG->sMCUs[3*X_DCTSIZE];
        ...
        for (iRow=0; iRow<4; iRow++)
        {
            for (iCol=0; iCol<4; iCol++)
            {
                ...
                if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
                    ...
                    JPEGPixelBE(pOutput+iCol, Y1, Cb, Cr);
                Y1 = (pY[X_DCTSIZE*2] + pY[X_DCTSIZE*2+1] + pY[X_DCTSIZE*2+8] + pY[X_DCTSIZE*2+9]) << 10;
    ...
    { // draw a 2x4 block
        ...
        pY += X_DCTSIZE*2; // next Y block below
        Y1 = pY[0] << 12;
        Y2 = pY[2] << 12;
        Cb = pCb[2];
        Cr = pCr[2];
...
static void JPEGPutMCU21(JPEGIMAGE *pJPEG, int x, int iPitch)
{
    ...
    { // draw 2 pixels
        ...
        Y2 = (signed int)(pY[X_DCTSIZE*2]) << 12;
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
    ...
    { // draw 4x2 pixels
        ...
        iCr = pCr[1];
        iCb = pCb[1];
        Y1 = (signed int)pY[X_DCTSIZE*2] << 12;
        Y2 = (signed int)pY[X_DCTSIZE*2+1] << 12;
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
        ...
        iCr = pCr[3];
        iCb = pCb[3];
        Y1 = (signed int)pY[X_DCTSIZE*2+2] << 12;
        Y2 = (signed int)pY[X_DCTSIZE*2+3] << 12;
        if (pJPEG->ucPixelType == RGB565_LITTLE_ENDIAN)
...
static int DecodeJPEG(JPEGIMAGE *pJPEG)
{
    ...
    iQuant1 = pJPEG->sQuantTable[pJPEG->JPCI[0].quant_tbl_no*X_DCTSIZE]; // DC quant values
    iQuant2 = pJPEG->sQuantTable[pJPEG->JPCI[1].quant_tbl_no*X_DCTSIZE];
    iQuant3 = pJPEG->sQuantTable[pJPEG->JPCI[2].quant_tbl_no*X_DCTSIZE];
