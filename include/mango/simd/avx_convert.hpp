...
namespace mango::simd
{
    ...
    inline s32x8 convert<s32x8>(f32x8 s)
    {
        ...
        auto lo = _mm256_extractf128_si256(temp, 0);
        auto hi = _mm256_extractf128_si256(temp, 1);
        return { lo, hi };
    }
    ...
    inline f32x8 convert<f32x8>(s32x8 s)
    {
        __m256i temp = _mm256_setr_m128i(s.data[0], s.data[1]);
    ...
    inline u32x8 convert<u32x8>(f32x8 s)
    {
        f32x4 lo_0 = _mm256_extractf128_ps(s, 0);
        f32x4 hi_0 = _mm256_extractf128_ps(s, 1);
        auto lo = convert<u32x4>(lo_0);
        auto hi = convert<u32x4>(hi_0);
        return { lo, hi };
    }
    ...
    inline f32x8 convert<f32x8>(u32x8 s)
    {
        __m128 lo = convert<f32x4>(s.data[0]);
        __m128 hi = convert<f32x4>(s.data[1]);
